# ============================================================
#  app.R  -  Children's Fitness Assessment Dashboard  v2.0
#
...
local({
  ...
  source("model fitting functions.R", local = FALSE)
})
...
# safe_diagnostics: dashboard-specific diagnostic extraction.
# Does NOT call extract_diagnostics() (which was designed for simulated data
# and calls quality_metrics() + get_rmse_table() that assume all four outcomes
# exist AND that ground-truth columns are present). Instead we build only what
# the dashboard displays — residual summaries, PI coverage, centile calibration
# — outcome-by-outcome so a missing or failed model never breaks the others.
safe_diagnostics <- function(df, fit_res) {
  ...
  # resid(m, type="quantile") silently fails for models fitted with re() random
  # effects, so we compute them manually from the fitted distribution parameters:
  #   r_q = qnorm( F(y ; mu, sigma) )
  # For LOGNO this simplifies to (log(y) - mu) / sigma.
  # For other families we use the pFAM CDF function.
  get_qr <- function(m, y_obs = NULL) {
    if (is.null(m)) return(NULL)
    
    # Try the built-in method first (works for non-mixed models)
    r <- tryCatch(as.numeric(resid(m, type = "quantile")), error = function(e) NULL)
    if (!is.null(r) && sum(is.finite(r)) > 10) return(r[is.finite(r)])
    
    # Manual fallback: need observed y
    ...
    # Align lengths (fitted values may drop NAs)
    n_fit <- length(mu)
    ...
    r <- if (fam == "LOGNO") {
      # r_q = qnorm(pnorm((log(y)-mu)/sigma)) = (log(y)-mu)/sigma
      y_pos <- pmax(y_obs, 1e-9)
      (log(y_pos) - mu) / pmax(sig, 1e-9)
    } else {
      # Generic: use pFAM CDF
  ...
  # Helper: one-row residual summary
  ...
  # Helper: PI coverage for a gamlss model via qFAM
  ...
  # Helper: PI coverage for Log-Normal (LOGNO) without qFAM lookup
  ...
  # Helper: centile calibration for Log-Normal
  ...
  # Helper: centile calibration via qFAM
  ...
  # PI coverage (LOGNO models via direct formula; NBI via qFAM)
    ...
    tabBox(width=12, id="main_tabs",
           ...
           tabPanel(title=tagList(icon("search")," Heterogeneity"),
                    ...
                    fluidRow(
                      ...
                      # ── UPDATED: variance decomp plot, zoomed to 80–100% ──
           ...
           tabPanel(title=tagList(icon("file-alt")," Summary for Teachers"),
                    ...
                    tags$div(style="max-width:960px;margin:0 auto;padding:10px;",
                             ...
                             # Score vs Age plot (replaces time-point trajectory)
...
server <- function(input, output, session) {
  ...
  output$plt_age_score <- renderPlot({
    ...
    # Connect each child's repeated measurements with a thin grey line
    p <- p +
  ...
  # ── UPDATED: variance decomp — zoomed to 80–100% ─────────
  output$plt_icc_bar <- renderPlot({
    vd <- var_decomp(); req(vd)
    
    # ── Reshape to long: only the two BETWEEN-level components ──────────────
    # We deliberately exclude pct_residual from the bars.
    # Instead individual variance is shown as a grey annotation per outcome,
    # so the school (red) and cohort/class (orange) bars get the full y-axis.
    pd <- vd %>%
    ...
    # Annotation: individual variance label to show below bars
    annot <- vd %>%
    ...
    ggplot(pd, aes(x = outcome, y = pct, fill = level)) +
      ...
      # Direct value labels inside bars
      geom_text(aes(label = sprintf("%.1f%%", pct)),
                position = position_stack(vjust = 0.5),
                colour = "white", fontface = "bold", size = 3.8) +
      # Grey annotation showing how much variance sits at individual level
  ...
  output$ts_traj_plot <- renderPlot({
    ...
    # Thin per-child lines if requested
