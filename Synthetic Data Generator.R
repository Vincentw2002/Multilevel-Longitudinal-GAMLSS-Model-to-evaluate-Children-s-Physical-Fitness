...
apply_scenarios_fitness <- function(dat,
                                    scenario = c("S1","S2","S3","S4","S5","S6","S8"),
                                    seed=1) {
  ...
  # This is lighter than S8 (which applies 100% heaping AND school calibration bias).
  if (scenario == "S4") {
  ...
  # S8: Dash/VC device/administrator calibration bias + rounding/heaping
  if (scenario == "S8") {
    set.seed(seed + 80)
